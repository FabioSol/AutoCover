...
import WUN_Sep.data.utils as data_utils
import WUN_Sep.model.utils as model_utils
import utils
...
def predict_song(args, audio_path, model):
    ...
    sources = {key : data_utils.resample(sources[key], args.sr, mix_sr) for key in sources.keys()}
...
def evaluate(args, dataset, model, instruments):
    ...
    with torch.no_grad():
        for example in dataset:
            ...
            target_sources = np.stack([data_utils.load(example[instrument], sr=None, mono=False)[0].T for instrument in instruments])
